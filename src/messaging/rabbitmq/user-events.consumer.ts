...
async function getUserEvents() {
    try {
        ...
        const connection: amqp.Connection = await getRabbitMQConnection();
            ...
            if (message) {
                try {
                    ...
                    if (success) {
                        ...
                        logger.warn(`[RabbitMQ] Processing failed for userID: ${userData.userID}`);
                    }
                } catch (error) {
...
async function processUserData(userData: UserData): Promise<boolean> {
    ...
    try {
        ...
        } else if (userData.type === "username" && userData.username) {
            return await updateUsername(userData.userID, userData.username);
        } else if (userData.type === "picture" && userData.profilePicture) {
